...
locale_row_new (const gchar *text)
{
  ...
  row = gtk_list_box_row_new ();
  widget = padded_label_new (text, ROW_LABEL_POSITION_CENTER, ROW_TRAVEL_DIRECTION_FORWARD, FALSE);
  gtk_container_add (GTK_CONTAINER (row), widget);
...
set_fixed_size (GtkWidget *chooser)
{
  ...
  gtk_scrolled_window_set_min_content_height(GTK_SCROLLED_WINDOW (priv->scrolledwindow), 150);
  ...
  gtk_window_get_size (gtk_window_get_transient_for (GTK_WINDOW (chooser)), &width, NULL);
  gtk_window_get_size (GTK_WINDOW (chooser), NULL, &height);
  gtk_widget_set_size_request (chooser, width * MAIN_WINDOW_WIDTH_RATIO, height+1);
